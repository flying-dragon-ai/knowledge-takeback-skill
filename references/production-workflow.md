...
The skill includes a reusable helper at `assets/magazine-bg-webgl.js`. The Editorial seed loads it as `../assets/magazine-bg-webgl.js`, assuming the generated `index.html` lives in a task folder one level below the skill root. If you place the generated HTML elsewhere, copy or inline the helper, or adjust the script path before rendering.
