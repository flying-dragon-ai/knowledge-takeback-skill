...
For screenshot exports, prefer `frozenTime` so repeated renders look stable. The seed template assumes the generated `index.html` lives one directory below the skill root, so it loads `../assets/magazine-bg-webgl.js`. If you place the generated HTML elsewhere, either copy the helper into the task folder or adjust the script path.
